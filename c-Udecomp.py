...
def c_Udecomp(Umatrix):
	if (Umatrix[0] == 0.0):
		if(Umatrix[3] != 0.0):
			print "Error0: Detected Unitary error"
			exit(1)
		gamma = m.pi
		beta = 0.0
		...
		if (U_1[0] != 1 or U_2[0] != 1):
			print "Error01: Detected unitary error"
		...
		if (U_0[0] != 1 or U_3[0] != 1):
			print "Error02: Detected unitary error"
...
z = [(1.0/m.sqrt(2)+0.0j),(1.0/m.sqrt(2)+0.0j),(1.0/m.sqrt(2)+0.0j),(-1.0/m.sqrt(2)+0.0j)]
(alpha,A,B,C) = c_Udecomp(z)
self_check(z,alpha,A,B,C)
